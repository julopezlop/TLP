...
class Juego:
    ...
    def starter(self):
        ...
        self.tabla_puntuacion_ancho = 3 # Definidido predeterminadamente
        ...
        if self.game_name == 'tetris':
            ...
            # Inicializar serpiente en el centro del tablero, moviéndose a la derecha
            ...
            self.direccion = [0, 1]  # [dy, dx] - derecha
            self.direccion_pendiente = None  # Para evitar giros de 180 grados
    ...
    def cargar_controles_snake(self):
        controls = self.rules.get('regla_controles', {})
        self.control_arriba = controls.get('rotar', 'w')  # w
        self.control_abajo = controls.get('acelerar_caida', 's')  # s
        self.control_izquierda = controls.get('izquierda', 'a')  # a
        self.control_derecha = controls.get('derecha', 'd')  # d
    ...
    def imprimir_tablero_snake(self):
        ...
        print("+" + "-" * self.ancho + "+")
        print(f"PUNTUACIÓN: {self.game_puntuacion} | Longitud: {len(self.serpiente)}")
    ...
    def run_game_tetris(self):
        ...
        print("\n¡JUEGO TERMINADO!")
        print(f"PUNTUACIÓN FINAL: {self.game_puntuacion}")
    ...
    def run_game_snake(self):
        ...
        while not self.juego_terminado:
            ...
            if msvcrt.kbhit():
                ...
                if key == self.control_arriba:
                    self.cambiar_direccion_snake([-1, 0])  # Arriba
                elif key == self.control_abajo:
                    self.cambiar_direccion_snake([1, 0])   # Abajo
                elif key == self.control_izquierda:
                    self.cambiar_direccion_snake([0, -1])  # Izquierda
                elif key == self.control_derecha:
                    self.cambiar_direccion_snake([0, 1])   # Derecha
                elif key == 'q':
                    self.juego_terminado = True
        ...
        print("\n¡JUEGO TERMINADO!")
        print(f"PUNTUACIÓN FINAL: {self.game_puntuacion}")
        print(f"Longitud final de la serpiente: {len(self.serpiente)}")